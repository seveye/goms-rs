use tokio::{net::ToSocketAddrs, sync::{mpsc, Mutex}};
use crate::watch::connect::{TcpReader, TcpWriter};
use std::{collections::HashMap, sync::Arc};

use super::{Message, message::new_message};
...
impl Client {
    ...
    pub async fn initialize(&mut self) -> Result<Message, std::io::Error> {
        let mut msg = new_message(String::from("initialize"), 0, self.keys.clone());
        self.send_message(&mut msg).await
    }

    pub async fn hget(&mut self, key: &str, field: &str) -> Result<String, std::io::Error> {
        let mut msg = new_message(String::from("hget"), 0, vec![String::from(key), String::from(field)]);
    ...
    pub async fn hset(&mut self, key: &str, field: &str, value: &str) -> Result<(), std::io::Error> {
        let mut msg = new_message(String::from("hset"), 0, vec![String::from(key), String::from(field), String::from(value)]);
        match self.send_message(&mut msg).await {
            Ok(msg) => {
                if msg.values.len() > 0 {
                    Ok(())
    ...
    pub async fn hincrby(&mut self, key: &str, field: &str, value: i64) -> Result<i64, std::io::Error> {
        let mut msg = new_message(String::from("hincrby"), 0, vec![String::from(key), String::from(field), value.to_string()]);
        match self.send_message(&mut msg).await {
            Ok(msg) => {
                if msg.values.len() > 0 {
